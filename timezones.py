...
class Timezones(commands.Cog):
        ...
        if country=="":
            ...
            for code, check_country in pytz.country_names.items():
                if check_country.lower() == country.lower():
                    return await self.send_city_shortlist(ctx.author, check_country)
            ...
            if contain_list != []:
                if len(contain_list) < 25:
                    return await self.send_country_shortlist(ctx.author, contain_list)
                else:
    ...
    async def send_country_shortlist(self, user, country_list):
        option_list = []
        for country in country_list:
            new_option = SelectOption(label=country, value=f"tz_country_{country}")
            option_list.append(new_option)
        
        return await user.send("I found some results containing that.\nPlease select your country / region.", components=[Select(placeholder="Select Country/Region", options=option_list)])
    
    async def send_city_shortlist(self, user, country):
        for code , check_country in pytz.country_names.items():
            if check_country.lower() == country.lower():
                option_list = []
                ...
                return await user.send(content="Select the city which follows your timezone.", components=[Select(placeholder="Select City", max_values=1, options=option_list)])
                    
        return await user.respond(content="Something went wrong, sorry. I have no idea what")
    ...
    async def on_select_option(self, interaction):
        ...
        if interaction.values[0][0:len(PREFIX_COUNTRY)] == PREFIX_COUNTRY:
            ...
            await self.send_city_shortlist(interaction.user, country)
